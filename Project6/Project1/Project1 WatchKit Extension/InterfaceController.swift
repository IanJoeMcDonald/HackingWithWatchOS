...
class InterfaceController: WKInterfaceController {
    ...
    override func contextForSegue(withIdentifier segueIdentifier: String, in table: WKInterfaceTable, rowIndex: Int) -> Any? {
        return ["index": String(rowIndex + 1), "totalNumber": String(notes.count), "note": notes[rowIndex]]
    }
    ...
    @IBAction func addNewNote() {
        ...
        presentTextInputController(withSuggestions: ["Hello"],  allowedInputMode: .plain) { [unowned self] result in
    ...
    @IBAction func addAudioNote() {
        let name = "Audio Note: " + String(describing: Date()) + ".wav"
        ...
        presentAudioRecorderController(withOutputURL: saveURL, preset: .narrowBandSpeech,
                                       options: options) { success, error in
                                        if success {
                                            self.table.insertRows(at: IndexSet(integer: self.notes.count), withRowType: "Row")
                                            // 4: give out new row the correct text
                                            self.set(row: self.notes.count, to: name)
                                            // 5: append the new niote to our array
